(function ($) {
  ...
  const left = document.querySelectorAll(
    ".outer-thumbnail, .highlight-4-shape-2"
  );
  if (left.length > 0) {
  ...
  const elements = document.querySelectorAll(
    ".highlight-2-shape-3, .highlight-1-shape-1, .highlight-4-shape-1"
  );
  ...
  const zoom = document.querySelectorAll(
    ".highlight-3-shape-1, .highlight.responsive .thumbnail"
  );
  if (zoom.length > 0) {
